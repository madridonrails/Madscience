...
module ModelAutoCompleterFilterHelper
  ...
  def belongs_to_auto_completer_filter(object, association, method, options={}, tag_options={}, completion_options={})
    class_name = object.to_s.camelize
    klass = class_name.constantize
    real_object = klass.new
    foreign_key = klass.reflect_on_association(association).primary_key_name

    tf_name = "filter[#{association}_#{method}]"
    tf_value = @filter["#{association}_#{method}"] rescue nil
    hf_name = "filter[#{foreign_key}]"
    hf_value = @filter["#{foreign_key}"] rescue nil
    options  = {
      :action => "auto_complete_belongs_to_for_filter_#{object}_#{association}_#{method}"
    }.merge(options)
    model_auto_completer_filter(tf_name, tf_value, hf_name, hf_value, options, tag_options, completion_options)
  end
  ...
  def model_auto_completer_result_filter(models, display, phrase=nil)
  ...
  #   is explained above.
  def model_auto_completer_filter(tf_name, tf_value, hf_name, hf_value, options={}, tag_options={}, completion_options={})
    options = {
      :regexp_for_id        => '(\d+)$',
      ...
      :action               => 'auto_complete_model_for_filter_' + tf_name.sub(/\[/, '_').gsub(/\[\]/, '_').gsub(/\[?\]$/, ''),
